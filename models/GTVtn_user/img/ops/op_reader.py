...
class DataLoader(Operator):
    ...
    def compute(self, op_input: InputContext, op_output: OutputContext, context: ExecutionContext):
        ...
        gtv_t_path = os.path.join(in_path, "GTV T.nii.gz")
        if os.path.isfile(gtv_t_path):
            gtv_t_arr = sitk.GetArrayFromImage(sitk.ReadImage(gtv_t_path))
            user_input = np.zeros_like(gtv_t_arr)

        gtv_n_path = os.path.join(in_path, "GTV N.nii.gz")
        if os.path.isfile(gtv_n_path):
            gtv_n_arr = sitk.GetArrayFromImage(sitk.ReadImage(gtv_t_path))
            user_input = np.zeros_like(gtv_n_arr)
